...
def generate_chapters(repository_path, target_path, book, meta, chapter_id, lang="fr"):
    print(f"  Generating chapters for language: {lang}")
...
def generate_chapter(
    ...
    with tempfile.NamedTemporaryFile() as metadata_file:
        ...
        html_content = generate_html_content(
            repository_path, current_chapter, metadata_file, lang
        )
    ...
    lang_dir = target_path / lang
    lang_dir.mkdir(exist_ok=True)

    (lang_dir / f"{current_chapter.id}.html").write_text(html_content)
...
def generate_html_content(
    ...
    md_file = (textes_path / chapter_id / f"{chapter_id}_{lang}.md")
    
    #fallback vers le fichier sans langue
    if not md_file.exists():
        md_file = textes_path / chapter_id / f"{chapter_id}.md"
...
def generate_homepage(repository_path, target_path, templates_folder, book, meta, lang="fr"):
    print(f"Rendering {target_path}:")
    print("  Generating homepage")

    source = repository_path / "textes" / "garde" / f"homepage_{lang}.md"
    ...
    # Créer un dossier par langue et écrire dedans
    lang_dir = target_path / lang
    lang_dir.mkdir(exist_ok=True)
    
    (lang_dir / "index.html").write_text(html_content)
    print(f"    Homepage written to: {lang_dir / 'index.html'}")
...
def generate_pdf(repository_path, template_path, target_path, book):
